...
class TaxonomyConversation(Base):
    ...
    label_space = Column(JSONB, nullable=True, default=list)  # Accumulated list of species/taxa
...
class TaxonomyMessage(Base):
    ...
    message_metadata = Column(JSONB, nullable=True)  # "metadata" is reserved by SQLAlchemy Declarative API
    created_at = Column(DateTime(timezone=True), server_default=func.now())
